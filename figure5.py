...
def plot_simple_dirac_spectrum(sorted_eig_vals, m=1):
    ...
    ax_main.add_patch(
        Rectangle((np.min(nneglam) - 0.05, -0.01), np.max(nneglam) - np.min(nneglam) + 0.1, np.max(rho_nneg) + 0.02,
...
def hypergraph_SBM(V, C, K, c1=15., c2=1.,seed=33):
    ...
    for i in range(C):
        ...
        for j in range(i+1, C):
            ...
            rng_j = np.random.default_rng(seed+j * 100)
...
if __name__=='__main__':
    ...
    sigma =140  # coupling strength (used in full dynamic version)
    # hyperedge size per community (here all =3, but length=10 unused)
    k = list(np.ones(4,dtype=int)*4)

    # --------------------------------------------------------
    # Generate hypergraph SBM instance
    # --------------------------------------------------------
    hyper_edges, B, flag,Inter_edges,dict_Inter_edges = hypergraph_SBM(N,C,k,c1=37,c2=1.2,seed = seed)
    nrun = 0
    ...
    for i in range(C):
        for j in range(N):
            ...
            axis[0, 1].plot(time[-2000:], theta_dot[-2000:],linewidth=1.0, color=color_list[i])
    ...
    for i in range(len(Inter_edges)-1):
        for j in range(N*C+Inter_edges[i],N*C+Inter_edges[i+1]):
            ...
            axis[1,1].plot(time[-2000:], theta_dot[-2000:], color=color_list[i],linewidth=1.0)
